...
const K = {
  products: 'f2g_products_v2',
  reels: 'f2g_reels_v2',
  orders: 'f2g_orders_v2'
}
...
export const seedProducts = [
  ...
  { id:'p2', name:'Indigo Everyday Kurti', category:'Kurtis & Sets', price:899, oldPrice:1099, image:'https://images.unsplash.com/photo-1583391733956-6c78276477e2?auto=format&fit=crop&w=900&q=84', badge:'Bestseller', description:'Easy everyday kurti for daily styling.', sizes:['S','M','L','XL','XXL'], active:true },
...
const toProduct = p => ({
  name:p.name, category:p.category, price:Number(p.price||0), old_price:p.oldPrice?Number(p.oldPrice):null,
  image:p.image, badge:p.badge||null, description:p.description||'', sizes:p.sizes||[], active:p.active!==false
})
...
export async function saveProduct(product){
  if (cloudEnabled){
    ...
    if(product.id && !String(product.id).startsWith('p')) payload.id=product.id
    const {data,error}=await supabase.from('products').upsert(payload).select().single()
    if(error) throw error
    return fromProduct(data)
  }
  const list=getLocal(K.products,seedProducts)
  const item={...product,id:product.id||uid()}
  const next=list.some(x=>x.id===item.id)?list.map(x=>x.id===item.id?item:x):[item,...list]
  setLocal(K.products,next)
  return item
}
...
export async function saveReel(reel){
  if(cloudEnabled){
    const payload={title:reel.title,url:reel.url,cover:reel.cover,active:reel.active!==false}
    if(reel.id && !String(reel.id).startsWith('r')) payload.id=reel.id
    const {data,error}=await supabase.from('reels').upsert(payload).select().single(); if(error) throw error; return data
  }
  const list=getLocal(K.reels,seedReels); const item={...reel,id:reel.id||uid()};
  const next=list.some(x=>x.id===item.id)?list.map(x=>x.id===item.id?item:x):[item,...list]; setLocal(K.reels,next); return item
}
...
const NOTE_PREFIX='__F2G_ORDER__'
function packNote(order){
  return NOTE_PREFIX+JSON.stringify({
    customer_note:order.customer_note||order.note||'', address:order.address||'', city:order.city||'',
    pincode:order.pincode||'', payment_method:order.payment_method||'COD', delivery_type:order.delivery_type||'Online Delivery',
    items:Array.isArray(order.items)?order.items:[], quantity:Number(order.quantity||1)
  })
}
function unpackNote(note){
  if(!note?.startsWith(NOTE_PREFIX)) return {customer_note:note||'',address:'',city:'',pincode:'',payment_method:'COD',delivery_type:'Enquiry',items:[],quantity:1}
  try{return {...JSON.parse(note.slice(NOTE_PREFIX.length)),raw_note:note}}catch{return {customer_note:note,address:'',city:'',pincode:'',payment_method:'COD',delivery_type:'Order',items:[],quantity:1}}
}
function fromOrder(row){return {...row,...unpackNote(row.note)}}
function toCloudOrder(order){return {
  customer_name:order.customer_name,
  phone:order.phone,
  size:order.size||'',
  product_id:order.product_id||null,
  product_name:order.product_name||'Online order',
...
export async function loadOrders(){
  if(cloudEnabled){ const {data,error}=await supabase.from('orders').select('*').order('created_at',{ascending:false}); if(!error&&data) return data.map(fromOrder) }
  return getLocal(K.orders,[])
}
export async function createOrder(order){
  const full={...order,id:uid(),status:order.status||'New',created_at:new Date().toISOString(),delivery_type:order.delivery_type||'Online Delivery'}
  if(cloudEnabled){ const {data,error}=await supabase.from('orders').insert(toCloudOrder(full)).select().single(); if(error) throw error; return fromOrder(data) }
  setLocal(K.orders,[full,...getLocal(K.orders,[])]); return full
}
export async function saveOrder(order){
  if(!order.id) return createOrder(order)
  if(cloudEnabled){ const {data,error}=await supabase.from('orders').update(toCloudOrder(order)).eq('id',order.id).select().single(); if(error) throw error; return fromOrder(data) }
  const list=getLocal(K.orders,[]); const item={...order,updated_at:new Date().toISOString()}; setLocal(K.orders,list.map(x=>x.id===item.id?item:x)); return item
}
...
export async function removeOrder(id){
  if(cloudEnabled){ const {error}=await supabase.from('orders').delete().eq('id',id); if(error) throw error; return }
  setLocal(K.orders,getLocal(K.orders,[]).filter(x=>x.id!==id))
}
